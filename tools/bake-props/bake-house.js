// Bake the Sketchfab "Forest House" GLB down to vertex-colour geometry
// (models/house.js), so it can render in creative mode, which has no texture
// loader by design.
//
// The source is a 10MB textured mid-poly scene: 12 PNG atlases, ~19.6k verts,
// house + trees + rocks + grass + animals. We keep only the house and the
// birch trees, sample each texture per vertex, and drop the images entirely.
// Run: node tools/bake-props/bake-house.js
...
const file = path.join(__dirname, 'forest_house.glb');
...
// Which materials belong to which output. Grass planes, animals and the ground
// disc are dropped: alpha-cutout foliage cannot survive a vertex-colour bake,
// and the scene supplies its own ground.
const HOUSE = ['House', 'Window', 'WoodPlanks', 'RoofMoss', 'WoodFence'];
const TREES = ['BrichTree', 'TreeLeafs'];
const parts = { house: [], trees: [] };
...
function walk(nodeIdx, parent) {
  ...
  if (node.mesh !== undefined) {
    j.meshes[node.mesh].primitives.forEach(p => {
      ...
      const bucket = HOUSE.indexOf(name) >= 0 ? 'house' : (TREES.indexOf(name) >= 0 ? 'trees' : null);
      if (!bucket) return;
...
const out = {
  house: build(parts.house, 'house'),
  trees: build(parts.trees, 'trees')
};
const js = '// Forest house — baked from a Sketchfab GLB to vertex colours, because\n' +
  '// creative mode has no texture loader. Textures are sampled per vertex at\n' +
  '// bake time and then discarded. Rebake: node tools/bake-props/bake-house.js\n' +
  'window.DZ_HOUSE = ' + JSON.stringify(out) + ';\n';
fs.writeFileSync(path.join(__dirname, '../../models/house.js'), js);
console.log('written models/house.js', (js.length / 1024).toFixed(0) + 'KB  (source GLB is 9910KB)');
